...
class SensorFusionNode(Node):
    ...
    def car_angles_callback(self, msg):
        ...
        self.x += displacement * np.cos(np.radians(self.theta_odom + self.orientation_change / 2))
        self.y += displacement * np.sin(np.radians(self.theta_odom + self.orientation_change / 2))
    ...
    def imu_listener_callback(self, msg):
        ...
        if self.theta_imu is not None:
            if self.orientation_change > 0:  # Turning Right
                ...
                self.theta = self.fuse_angles(self.theta_odom, self.theta_imu, 1.0, 0.0)
